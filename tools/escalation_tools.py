...
When Vela is uncertain, detects sensitive keywords, or encounters complex
...
logger = logging.getLogger("vela.tools.escalation")
...
class SendEscalationTool:
    ...
    def description(self) -> str:
        return (
            "Send an escalation email to the attorney explaining why Vela "
            "could not auto-handle the request. Includes the original context, "
            "what Vela understood, and what it needs the attorney to decide."
        )
    ...
    def input_schema(self) -> dict:
        return {
            ...
            "properties": {
                ...
                "vela_understanding": {
                    "type": "string",
                    "description": "What Vela understood from the email",
                },
                "suggested_action": {
                    "type": "string",
                    "description": "What Vela recommends the attorney do",
                },
            ...
            "required": ["attorney_email", "original_subject", "reasons", "vela_understanding"],
        }
...
<div style="font-family: Calibri, Arial, sans-serif; color: #333;">
  ...
  <p>{input.get('vela_understanding', '')}</p>
  ...
  <p style="color: #888; font-size: 12px;">
    This message was generated by Vela-Law, your scheduling assistant.<br>
...
"""
        ...
        try:
            await self._graph.send_mail(
                ...
                subject=f"[Vela Escalation] {input.get('original_subject', '')}",
                body_html=body_html,
                cc_addresses=cc,
            )
            return ToolResult(
